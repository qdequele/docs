...
function updatePageNumber(elem) {
  ...
  let pageNumber = parseInt(
    document.querySelector(".pagination").dataset.pageNumber
  );
...
document.querySelector("#previous_button").onclick = function () {
...
```js
const results = await index.search("tarkovsky", {
  hitsPerPage: 25,
});
```
...
<Capsule intent="note">
  `hitsPerPage` and `page` take precedence over `offset` and `limit`. If a query
  contains either `hitsPerPage` or `page`, any values passed to `offset` and
  `limit` are ignored.
</Capsule>
